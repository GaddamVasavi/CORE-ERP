...
export const ManufacturingModuleViewPart10: React.FC = () => {
  const [searchTerm, setSearchTerm] = useState('');
  const [activeFilter, setActiveFilter] = useState('ALL');

  const chartData = [
    { period: 'W1', value: 104500, benchmark: 93000 },
    { period: 'W2', value: 126200, benchmark: 109200 },
    { period: 'W3', value: 120800, benchmark: 114800 },
    { period: 'W4', value: 156100, benchmark: 127500 },
  ];

  const tableRows = Array.from({ length: 8 }).map((_, idx) => ({
    id: `MAN-${202600 + idx + i * 10}`,
    title: `Enterprise MANUFACTURING Process Node ${idx + 1} - Stream ${i}`,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">MANUFACTURING — Enterprise Console (Part 10)</h2>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" icon={<Plus className="w-4 h-4" />}>New MANUFACTURING Record</Button>
        </div>
      </div>

      {/* KPI Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Active Operations" value="420" change="8.4%" isPositive icon={Layers} color="blue" />
        <StatCard title="Reconciled Volume" value="$18,50,000" change="12.1%" isPositive icon={CheckCircle} color="emerald" />
        <StatCard title="Pending Verifications" value="50" change="Within SLA" isPositive icon={RefreshCw} color="purple" />
        <StatCard title="Variance Index" value="0.010%" change="Optimal" isPositive icon={AlertTriangle} color="amber" />
      </div>
        ...
        <div className="h-64 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData}>
              <defs>
                <linearGradient id="colorVal10" x1="0" y1="0" x2="0" y2="1">
              ...
              <Tooltip />
              <Area type="monotone" dataKey="value" stroke="#2563eb" fillOpacity={1} fill="url(#colorVal10)" strokeWidth={2} name="Actual ($)" />
      ...
      <Card>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
          <div className="w-full md:w-80">
            <Input
              placeholder="Search manufacturing records by ID, title or tags..."
              value={searchTerm}
